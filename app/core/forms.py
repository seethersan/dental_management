...
from .models import Patient, Doctor, Procedure, Visit, Appointment, DoctorClinicAffiliation, Clinic, Country, State, City
...
class VisitForm(forms.ModelForm):
    ...
    visit_date = forms.DateTimeField(widget=forms.DateTimeInput(attrs={'type': 'datetime-local'}))  # For DateTime picker
...
class AppointmentForm(forms.ModelForm):
    ...
    procedure = forms.ModelChoiceField(queryset=Procedure.objects.all(), widget=forms.Select(attrs={'class': 'form-select'}))
    clinic = forms.ModelChoiceField(queryset=Clinic.objects.none(), widget=forms.Select(attrs={'class': 'form-select'}))  # Queryset set to none initially
    doctor = forms.ModelChoiceField(queryset=Doctor.objects.none(), widget=forms.Select(attrs={'class': 'form-select'}))  # Queryset set to none initially
    appointment_date = forms.DateTimeField(widget=forms.Select(attrs={'class': 'form-select'}))  # Dynamic based on available time slots

class DoctorForm(forms.ModelForm):
    username = forms.CharField(max_length=150, help_text="Enter username for the doctor", widget=forms.TextInput(attrs={'class': 'form-control'}))
    ...
    email = forms.EmailField(max_length=255, help_text="Enter email for the doctor", widget=forms.EmailInput(attrs={'class': 'form-control'}))
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'form-control'}), help_text="Enter password for the doctor")
...
class DoctorUpdateForm(forms.ModelForm):
    ...
    npi = forms.CharField(label="NPI", widget=forms.TextInput(attrs={'class': 'form-control'}))
...
class DoctorClinicAffiliationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        clinic = self.instance.clinic if self.instance.pk else cleaned_data.get('clinic')

        # Check if the doctor-clinic affiliation already exists
        if doctor and clinic:
            if DoctorClinicAffiliation.objects.filter(doctor=doctor, clinic=clinic).exclude(pk=self.instance.pk).exists():
                raise forms.ValidationError('This doctor is already affiliated with this clinic.')
...
class ClinicForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'state': forms.Select(attrs={'class': 'form-select', 'id': 'id_state', 'disabled': 'true'}),  # Disable by default
            'city': forms.Select(attrs={'class': 'form-select', 'id': 'id_city', 'disabled': 'true'})  # Disable by default
        }
...
class DoctorClinicAffiliationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if DoctorClinicAffiliation.objects.filter(doctor=doctor, clinic=clinic).exclude(pk=self.instance.pk).exists():
            raise forms.ValidationError(f'Doctor {doctor.user.get_full_name()} is already affiliated with this clinic.')
