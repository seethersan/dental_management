...
from .models import Doctor, Clinic, Procedure, DoctorClinicAffiliation, Patient, Visit, Appointment, State, City
from django.db.models import Count
from django.contrib.auth.mixins import LoginRequiredMixin
from .forms import ClinicForm, ClinicUpdateForm, DoctorForm, DoctorUpdateForm, PatientForm, PatientUpdateForm, VisitForm, AppointmentForm, DoctorClinicAffiliationForm
...
class PatientListView(ListView):
    ...
    def get_queryset(self):
        ...
        for patient in queryset:
            patient.last_visit = Visit.objects.filter(patient=patient).order_by('-visit_date').first()
            patient.next_appointment = Appointment.objects.filter(patient=patient).order_by('appointment_date').first()
        return queryset
...
class PatientCreateView(CreateView):
    ...
    success_url = reverse_lazy('patient-list')  # Redirect after successful patient creation
...
class PatientDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['visits'] = Visit.objects.filter(patient=self.object).order_by('-visit_date')
        context['next_appointment'] = Appointment.objects.filter(patient=self.object).order_by('appointment_date').first()
        return context
...
class PatientUpdateView(UpdateView):
    ...
    context_object_name = 'patient'
    success_url = reverse_lazy('patient-list')  # Redirect to patient list after successful update
...
class ClinicListView(ListView):
    ...
    def get_queryset(self):
        return Clinic.objects.annotate(
            ...
            num_patients=Count('doctorclinicaffiliation__doctor__doctorpatientaffiliation__patient', distinct=True)
        )
...
def create_affiliation(request, clinic_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'core/create_affiliation.html', {'form': form, 'clinic': clinic})
    else:
...
def edit_affiliation(request, affiliation_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            affiliation.save()
            return redirect('clinic-detail', pk=affiliation.clinic.id)  # Redirect to clinic detail
    else:
    ...
    return render(request, 'core/edit_affiliation.html', {
        'form': form,
...
def schedule_appointment(request, patient_id):
    ...
    if request.method == 'POST':
        ...
        if clinic_id:
            form.fields['doctor'].queryset = Doctor.objects.filter(clinics__id=clinic_id)
    ...
    return render(request, 'core/schedule_appointment.html', {'form': form, 'patient': patient})

def ajax_load_clinics(request):
    procedure_id = request.GET.get('procedure_id')
    clinics = Clinic.objects.filter(doctorclinicaffiliation__doctor__specialties__id=procedure_id).distinct()
...
def ajax_load_doctors(request):
    ...
    doctors = Doctor.objects.filter(clinics__id=clinic_id, specialties__id=procedure_id).distinct()
    doctor_data = [{'id': doctor.id, 'name': doctor.user.get_full_name()} for doctor in doctors]
...
def ajax_load_procedures(request):
    ...
    procedure_data = [{'id': procedure.id, 'name': procedure.name} for procedure in procedures]
...
def ajax_load_timeslots(request):
    ...
    booked_appointments = Appointment.objects.filter(doctor_id=doctor_id, clinic_id=clinic_id).values_list('appointment_date', flat=True)
